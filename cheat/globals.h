...
namespace globals
{
	...
	inline float glowColor[] = {1.f, 0.f, 0.f, 0.8f};

	inline bool Chams = false;
	inline bool HPChams = false;
	inline float Brightness[] = {2.5f};
	inline float ChamsColor[] = {0.f, 1.f, 1.f};

	inline bool aimbot = false;
	inline float Smoothening[] = {15.0f};
	inline float FOV[] = {3.0f};
	inline int AimPart = 8;

	inline int AimKey = VK_LBUTTON;

	inline bool Triggerbot = false;
	inline int TriggerKey = VK_SHIFT;
	...
	inline bool LessFlash = false;
	inline float FlashPer[] = {100.0f};

	inline bool spec = false;
	inline bool hasSpectators = false;

	inline bool Radar = false;

	inline bool gameFOV = false;
	inline int gameFOVslider = 120.0;
}
...
namespace offsets
{
	...
	constexpr ::std::ptrdiff_t find_hud_element = 0x28070090;
...
inline char* GetPlayerName(int index, const Memory& mem)
{
	uintptr_t dwClientState = mem.Read<uintptr_t>(globals::engineAddress + offsets::dwClientState);
	uintptr_t UserInfoTable = mem.Read<uintptr_t>(dwClientState + offsets::dwClientState_PlayerInfo);
	uintptr_t items = mem.Read<uintptr_t>(mem.Read<uintptr_t>(UserInfoTable + 0x40) + 0xC);
	player_info_s info = mem.Read<player_info_s>(mem.Read<uintptr_t>(items + 0x28 + ((index) * 0x34)));
